...
import { ChatMessage, AIPreparedAssessment, DefenseQuestion } from "../types";
import { Mic, MicOff, Send, Sparkles, User, Play, AlertTriangle, Cpu, ChevronRight, Clipboard } from "lucide-react";
          ...
          {chatHistory.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex items-start gap-3 ${isStudent ? "justify-end" : "justify-start"}`}>
                {!isStudent && (
                  <div className={`w-8 h-8 rounded text-[9px] flex items-center justify-center font-bold uppercase shrink-0 ${
                    isInstructor 
                ...
                <div className={`p-4 rounded-xl max-w-[85%] text-left whitespace-pre-line text-xs relative ${
                  isStudent 
                  ...
                  <span className={`text-[9px] block mb-1 font-bold font-mono tracking-wide ${
                    isStudent ? "text-indigo-300" : isInstructor ? "text-emerald-400" : "text-white/40"
                  }`}>
            ...
            {role === "both" && assessmentMode === "instructor" && (
              <div className="flex gap-2 pb-1.5 border-b border-white/5 mb-1.5">
                ...
                <button
                  ...
                  className={`px-2.5 py-1 text-[9px] font-bold font-mono uppercase tracking-wide rounded transition cursor-pointer ${
                    sendAs === "student"
